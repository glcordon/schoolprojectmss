				...
				@if(isset(Session::get('tenant')->site_name))
							@foreach($sites->courses->toArray() as $course)
								...
								<div class="card col-4 shadow-sm" 
									style="text-align:left; margin-bottom:20px; background-image:url({{ $course['course_image'] ? Storage::url($course['course_image'])  ?? asset('img/baseball-field.jpg') : asset('img/baseball-field.jpg') }}); background-size:cover;">
									{{--  {{ dd($courses) }}  --}}
